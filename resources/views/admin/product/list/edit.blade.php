<x-app-layout>
    ...
    <x-breadcrumb :items="[
        ...
        ['route' => route('admin.product.list-product.edit', $data->id), 'name' => 'Edit Produk'],
    ]" />
    ...
    <x-card class="max-w-5xl mx-auto mb-4">
        ...
        <form action="{{ route('admin.product.list-product.update', $data->id) }}" method="POST" class="mt-5 space-y-6"
            ...
            <div>
                ...
                <x-file-input id="image" name="image" value="{{ route('admin.product.get-image', $data->image) }}"
                    class="mt-1 block w-full" />
                <x-input-error :messages="$errors->get('image')" class="mt-2" />

                <small class="mt-2 text-gray-700">Gambar telah diunggah: <a
                        href="{{ route('admin.product.get-image', $data->image) }}" target="_blank"
