<x-app-layout>
    ...
    <x-breadcrumb :items="[['route' => route('admin.attraction.index'), 'name' => 'Atraksi']]" />
    ...
    <x-card class="max-w-5xl mx-auto mb-4">
        ...
        <form action="{{ route('admin.attraction.update', $data->id) }}" method="POST" class="mt-5 space-y-6">
            @csrf
            @method('PUT')
        ...
        <div>
            <x-secondary-button class="mb-4" data-modal-target="modal-items" data-modal-toggle="modal-items">
                {{ __('Tambahkan Atraksi') }}
            </x-secondary-button>

            <p class="uppercase text-sm text-gray-700 my-4 font-semibold">Daftar Atraksi</p>

            @forelse ($listAttraction as $item)
                <div class="border border-gray-300 p-6 rounded-lg shadow-sm" id="listAttraction-{{ $item->id }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        @if ($item->image)
                            <small class="mt-2 text-gray-700">Gambar telah diunggah: <a
                                    href="{{ route('admin.attraction.get-image', $item->image) }}" target="_blank"
                    ...
                    <div class="mb-3">
                        <x-input-label for="description" :value="__('Deskripsi')" required />
                        <x-textarea id="description" name="description" class="mt-1 block w-full" :value="$item->description" />
                        <x-input-error :messages="$errors->get('description')" class="mt-2" />
                    </div>
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                    ...
                    <button type="button"
                        ...
                        data-modal-hide="modal-items-listAttraction">
                ...
                <form action="{{ route('admin.attraction.list-attraction.store') }}" method="POST" class="space-y-6"
        ...
        <script>
            function deleteItems(id) {
                if (confirm('Apakah Anda yakin ingin menghapus item ini?')) {
                    // disable button
                    $(`#listAttraction-${id} button[type="button"]`).prop('disabled', true).text('Processing...');

                    $.ajax({
                        url: `{{ route('admin.attraction.list-attraction.destroy', ':id') }}`.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
            ...
            function updateItems(id) {
                ...
                formData.append('title', $(`#listAttraction-${id} #title`).val());
                formData.append('description', $(`#listAttraction-${id} #description`).val());
                formData.append('items', $(`#listAttraction-${id} #items`).val());

                // Check if an image file is selected before appending
                const imageFile = $(`#listAttraction-${id} #image`)[0].files[0];
                if (imageFile) {
                ...
                $.ajax({
                    url: `{{ route('admin.attraction.list-attraction.update', ':id') }}`.replace(':id', id),
